...
Result file is 3 tab-delimited columns: peptide with/without mods, protein ID.
...
while (<$peps_fh>) {
    s/\r?\n//;
    my $peptide_with_mods = $_;
    my $peptide = $peptide_with_mods;
    # Remove any mods when searching the proteins
    $peptide =~ s/[^A-Za-z]+//g;
    next if $tried{$peptide}++; # no reason to search peptides more than once
	...
	foreach my $prot_id (@prot_ids) {
		...
		print $out_fh "$peptide_with_mods\t$peptide\t$prot_id\n";
		last;
	    }
	}
    }
	...
	foreach my $prot_id1 (@prot_ids1) {
		...
		if (index($prot_seq, $peptide) != -1) {
		    ...
		    print $out_fh "$peptide_with_mods\t$peptide\t$prot_id1\n";
		    last;
		}
	    }
	}
    ...
    if (! $found{$peptide}) {
	print $out_fh "$peptide_with_mods\t$peptide\t$Not_Found_String\n";
	push @not_found, $peptide_with_mods;
    }
}
close $peps_fh;
close $out_fh;
